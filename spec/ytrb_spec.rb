...
module Ytrb
  module ExampleGroupMethods
    def expect_template(templ, proc)
      describe templ.inspect do
        let(:template) { templ }
        it { expect(result).to instance_exec(&proc) }
      end
    end
  end
  ...
  RSpec.describe Engine do
    ...
    after { |ex| puts subject.src if ex.exception }
    ...
    describe "with a simple binding" do
      ...
      let(:b) { get_binding(1, "hello") }
      let(:result) { subject.result(b) }
    ...
    describe "use a template method multiple times" do
      ...
      it "parses to the expected object" do
        expect(YAML.safe_load(result)).to eq(expected)
      end
